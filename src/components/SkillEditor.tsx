...
export function SkillEditor({ proficient, expertise, onChange }: Props) {
  ...
  return <div className="check-grid">{skills.map((skill) => <div className="skill-choice" key={skill.id}>
    <label><input type="checkbox" checked={proficient.includes(skill.id)} onChange={() => toggleProficiency(skill.id)} /><span>{t(labelKey('skill', skill.id))}<small>{t(labelKey('ability', skill.ability))}</small></span></label>
    <label className="expertise"><input type="checkbox" disabled={!proficient.includes(skill.id)} checked={expertise.includes(skill.id)} onChange={() => toggleExpertise(skill.id)} /> {t('skill.expertise')}</label>
  </div>)}</div>;
}
