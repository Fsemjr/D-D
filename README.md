...
A interface, a ficha e o PDF suportam **Português do Brasil (pt-BR)** e **English (en-US)**. Use o seletor `PT-BR | EN` no topo; a preferência fica salva no LocalStorage e é aplicada sem recarregar a página.

A publicação esperada é <https://fsemjr.github.io/D-D/>. O workflow `.github/workflows/deploy-pages.yml` executa testes, lint e build antes de publicar `dist` em pushes para `main` ou por acionamento manual. Ative **GitHub Actions** como fonte do Pages nas configurações do repositório.

Para testar localmente com o mesmo base path:

```bash
npm run build
npm run preview
# abra http://localhost:4173/D-D/
```
